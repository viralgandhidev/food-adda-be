-- Reset keywords to align with updated CSV (KEYWORDSB2B (2).csv)
-- Approach: wipe keywords and insert curated lists per main category (order preserved)
...
-- Indian Snacks & Sweets (updated: removed 'A. Indian Snacks (Savory)' and 'B. Indian Sweets (Mithai & Desserts)')
INSERT INTO
    keywords (
        id,
        name,
...
FROM (
        SELECT 'Namkeen & Mixtures' AS name
        UNION ALL
        SELECT 'Bhujia'
        UNION ALL
        ...
        UNION ALL
        SELECT 'Regional Mixtures (South Indian, Bombay, North Indian)'
        UNION ALL
        SELECT 'Farsan & Regional Snacks'
        UNION ALL
        SELECT 'Kachori, Samosa, Mathri'
        UNION ALL
        SELECT 'Chakli / Murukku'
        UNION ALL
        SELECT 'Banana Chips, Tapioca Chips'
        UNION ALL
        SELECT 'Khakhra & Papad'
        UNION ALL
        SELECT 'Roasted & Baked Snacks'
        UNION ALL
        SELECT 'Roasted Chana / Peanuts'
        UNION ALL
        SELECT 'Makhana (Fox Nuts)'
        UNION ALL
        SELECT 'Baked Mathri / Khakhra'
        UNION ALL
        SELECT 'Extruded Snacks & Chips'
        UNION ALL
        SELECT 'Potato Chips'
        UNION ALL
        SELECT 'Kurkure-style Extruded Snacks'
        UNION ALL
        SELECT 'Popcorn'
        UNION ALL
        ...
        UNION ALL
        SELECT 'Barfi (Kaju Katli, Coconut, Besan, Badam, Chocolate)'
        UNION ALL
        SELECT 'Halwa (Sooji, Moong Dal, Gajar, Karachi Halwa)'
        UNION ALL
        SELECT 'Milk-based Sweets'
        UNION ALL
        SELECT 'Rasgulla, Gulab Jamun, Cham Cham'
        UNION ALL
        SELECT 'Rasmalai, Malai Sandwich'
        UNION ALL
        SELECT 'Kalakand, Milk Cake, Peda'
        UNION ALL
        SELECT 'Regional Specialties'
        UNION ALL
        SELECT 'Mysore Pak, Dharwad Peda, Sandesh, Mishti Doi'
        UNION ALL
        SELECT 'Petha, Ghevar, Balushahi'
        UNION ALL
        SELECT 'Modak, Kozhukattai, Poli'
        UNION ALL
        SELECT 'Modern & Fusion Sweets'
        UNION ALL
        SELECT 'Dry Fruit Rolls'
        UNION ALL
        SELECT 'Chocolate-dipped Mithai'
        UNION ALL
        SELECT 'Sugar-free / Low-calorie Sweets'
        UNION ALL
        SELECT 'Diwali Specials (Soan Papdi, Kaju Katli, Mix Mithai Boxes)'
        UNION ALL
        SELECT 'Holi Specials (Gujiya, Malpua, Thandai Mix)'
        UNION ALL
        SELECT 'Raksha Bandhan (Gift Packs, Premium Dry Fruit Mithai)'
        UNION ALL
        SELECT 'Eid Specials (Sheer Khurma Mix, Sewai, Baklava)'
        UNION ALL
        SELECT 'Christmas Specials (Plum Cake, Dry Fruit Cake)'
        UNION ALL
        SELECT 'Vacuum-packed Mithai (long shelf life)'
        UNION ALL
        SELECT 'Frozen Snacks (Samosa, Kachori, Patties)'
        UNION ALL
        SELECT 'Sweet Gift Hampers (Corporate, Wedding, Festive)'
        UNION ALL
        SELECT 'Namkeen Gift Packs'
    ) k;
